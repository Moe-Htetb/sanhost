...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ct4233 = isset($_POST['CT-4233']) ? trim($_POST['CT-4233']) : null;
    $ct4234 = isset($_POST['CT-4234']) ? trim($_POST['CT-4234']) : null;
    $ct4236 = isset($_POST['CT-4236']) ? trim($_POST['CT-4236']) : null;
    $cst4257 = isset($_POST['CST-4257']) ? trim($_POST['CST-4257']) : null;

    // Validate required fields
    if (empty($roll_no)) {
        $_SESSION['error'] = 'Roll number is required';
        header("Location: fourth2sem_ct.php");
        exit;
    }

    // Check if the roll number already exists in the results table
    $check_query = "SELECT * FROM fourth_2sem_ct WHERE roll_no = ?";
    ...
    $query = "INSERT INTO fourth_2sem_ct (roll_no, `CST-4211`, `CST-4242`, `CT-4233`, `CT-4234`, `CT-4236`, `CST-4257`)
              VALUES (?, ?, ?, ?, ?, ?, ?)";

    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, 'sssssss', $roll_no, $cst4211, $cst4242, $ct4233, $ct4234, $ct4236, $cst4257);
